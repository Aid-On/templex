...
const FEAR_DRIVEN_EN = `# Is Your Business Ready for the AI Revolution?
...
## Act Now
Early adopters gain competitive advantages that become insurmountable.
**Limited offer:** Free AI readiness assessment for the first 20 companies this quarter.`;

const FEAR_DRIVEN_JA = `# あなたのビジネスはAI革命に対応できていますか？

## 隠れた危機
競合他社がAIを活用している間、あなたはまだ検討中です。Fortune 500企業の80%がすでにAIを中核業務に統合しています。

## 身近な証拠
TechCorpという中規模企業の話をしましょう。AI トレンドを無視した結果、18ヶ月で市場シェアの40%を奪われました。
...
const PROBLEM_SOLUTION_EN = `# Solving Digital Transformation Challenges
...
Companies that fail to transform digitally lose 25% revenue growth compared to digital leaders.
...
const PROBLEM_SOLUTION_JA = `# デジタル変革の課題を解決する
...
function detectPattern(userPrompt: string): 'problem-solution' | 'fear-driven' | null {
  ...
  if (isProblemSolution) return 'problem-solution';
  const isFearDriven = userPrompt.includes('恐怖フック') || userPrompt.includes('Fear Hook');
  if (isFearDriven) return 'fear-driven';
  return null;
}

function detectEnglish(systemPrompt: string, userPrompt: string): boolean {
  return systemPrompt.includes('English') ||
    userPrompt.includes('Generate the article in English') ||
    userPrompt.includes('Language: en');
}

const RESPONSES: Record<string, Record<string, string>> = {
  'fear-driven': { en: FEAR_DRIVEN_EN, ja: FEAR_DRIVEN_JA },
  'problem-solution': { en: PROBLEM_SOLUTION_EN, ja: PROBLEM_SOLUTION_JA },
};
...
class PatternAwareMockProvider implements LLMProvider {
  async chat(systemPrompt: string, userPrompt: string, _options?: { temperature?: number; maxTokens?: number }): Promise<string> {
    const pattern = detectPattern(userPrompt);
    if (pattern) {
      const lang = detectEnglish(systemPrompt, userPrompt) ? 'en' : 'ja';
      return RESPONSES[pattern][lang];
    }
...
describe('Pattern-based Generation Integration', () => {
  ...
  describe('Fear-Driven Pattern', () => {
    it('should generate fear-driven article in Japanese', async () => {
      const result = await generator.generateFromPattern('fear-driven', {
        topic: 'AI導入', fearHook: '競合他社に遅れをとっていませんか？',
        evidence: 'Fortune 500の80%がAI導入済み', solution: '段階的なAI導入アプローチ',
        examples: [{ industry: '小売', company: 'Amazon', result: '売上35%増' }],
        japanContext: '日本企業の多くがまだ検討段階',
        urgency: '2024年第1四半期までに開始', cta: 'AIコンサルタントに相談'
      }, { language: 'ja' });
    ...
    it('should generate fear-driven article in English', async () => {
      const result = await generator.generateFromPattern('fear-driven-en', {
        topic: 'AI Adoption', fearHook: 'Are you falling behind competitors?',
        evidence: '80% of Fortune 500 using AI', solution: 'Phased AI implementation',
        examples: [{ industry: 'Retail', company: 'Amazon', result: '35% sales increase' }],
        urgency: 'Start by Q1 2024', cta: 'Contact AI consultants'
      }, { language: 'en' });
      expect(result).toBeDefined();
  ...
  describe('Problem-Solution Pattern', () => {
    it('should generate problem-solution article', async () => {
      const result = await generator.generateFromPattern('problem-solution-en', {
        topic: 'Digital Transformation', fearHook: 'Legacy systems holding you back?',
        evidence: 'Digital leaders grow 25% faster', solution: 'Phased transformation approach',
        examples: [{ industry: 'Finance', company: 'JP Morgan', result: '360,000 hours saved' }]
      }, { language: 'en' });
      expect(result).toBeDefined();
  ...
  describe('Pattern validation', () => {
    ...
    it('should handle unknown patterns gracefully', async () => {
      await expect(
        generator.generateFromPattern('non-existent-pattern', { topic: 'test' })
  ...
  describe('Custom options with patterns', () => {
    it('should apply custom temperature and maxTokens', async () => {
      ...
      const capturingProvider: LLMProvider = {
        chat: async (_sys, _user, options) => { capturedOptions = options; return '# Test Article'; }
      };
      const customGenerator = new ArticleGenerator(capturingProvider);
      await customGenerator.generateFromPattern('fear-driven', { topic: 'Test' }, {
        temperature: 0.3, maxTokens: 5000, language: 'en'
      });
  ...
  describe('Performance with patterns', () => {
    it('should generate multiple articles efficiently', async () => {
      const topics = ['AI in Healthcare', 'Blockchain Security', 'Cloud Migration', 'Data Analytics', 'Cybersecurity'];
      const startTime = Date.now();
      const results = await Promise.all(
        topics.map(topic => generator.generateFromPattern('fear-driven', {
          topic, fearHook: `Is your ${topic} strategy outdated?`, solution: `Modern ${topic} implementation`
        }, { language: 'en' }))
      );
